...
| Package | Version | Why it is here |
|---|---|---|
| `typescript` | 6.0.3 | `.steps.ts` files are typed, so authoring errors are build errors — the whole premise of §5. **Not 7.x:** the native compiler does not expose the programmatic API `astro check` needs (R-11). |
| `@astrojs/check` | 0.9.10 | Type-checks `.astro` files. Explicit rather than implicit: absent, `astro check` prompts to install it and **exits 0 in CI without checking anything** (R-15). |
...
| **R-11** | ~~TypeScript 7 is new.~~ **Fired at P1, as predicted, and resolved.** `astro check` failed outright: *"The TypeScript module loaded (found 7.0.2) does not expose the programmatic API that `astro check` relies on. TypeScript's native compiler (7.0 and later) does not ship this API yet."* Tracked at withastro/roadmap#1321. **Resolved by pinning `typescript@6.0.3`** — the planned fallback, one line, no code impact. `astro check` now reports 0 errors / 0 warnings / 0 hints across 15 files. Revisit when the native compiler exposes the API. |
| **R-15** | **A build gate that verifies nothing.** `astro check` prompts interactively to install `@astrojs/check` when it is absent. In a non-TTY it does not fail — it exits 0. `build:ci` therefore claimed to typecheck while checking nothing, and both the local build and the first Cloudflare deploy passed green. A gate that is silently inert is worse than a missing one, because it is trusted. | `@astrojs/check` is now an explicit pinned devDependency, so the prompt can never arise. General lesson recorded here: **a gate is not verified until it has been seen to fail.** The font verifier was proven against a deliberately broken subset for this reason; the same standard now applies to every check in `build:ci`. |
...
| **R-14** | **Quoted compiler output cannot fit a phone, and truncation is silent.** Measured at P1: a real `E0382` block is 100 columns at its widest. After the narrow-viewport work a 380px phone shows 52 and a 320px phone 43 — the headline fits, three of fourteen lines still do not. Wrapping is not available: rustc diagnostics are column-aligned and carets stop meaning anything once a line reflows. No type size reaches 100 columns while staying readable, so this cannot be solved in CSS. | Two parts. Mechanically, an edge shadow marks any block with content off-screen, and it appears *only* when there genuinely is more — with no overflow both cover layers hide both shadows, so its presence is never noise. Editorially, **`CLAUDE.md` now requires the Arabic prose to convey every critical line independently**: a reader who never scrolls still gets the lesson, and the verbatim text serves authenticity and search rather than comprehension. Enforced in the per-lesson definition of done. Lesson 2.2 step 6 is the case that matters — the cut line is the *reason* the move happened. |

---

## 9a. Shipped state — end of leave period 1

Recorded here so a later session inherits measurements instead of re-deriving
them. Everything below was measured, not estimated.

**Launch scope is complete.** 9 lessons, 5 problem pages, `MemoryStepper` with
2 visuals, 4 static SVGs, `/about` `/discuss` `/admin` `/problems`, sitemap,
RSS, 404, OG card, licences, issue template.

| Measurement | Value |
|---|---|
| `MemoryStepper` island, gzip | **10,620 B** against a 14 KB budget |
| Prose pages, external JS | **0 B** — every page except the two stepper lessons |
| Inline JS, prose pages | 309 B (the anti-FOUC theme script, C3) |
| Inline JS, stepper lessons | 5,061 B — 4,380 Astro island bootstrap + 372 `client:visible` shim + 309 ours |
| Lighthouse mobile, `/rust/ownership/move/` | perf **97**, a11y **100**, best practices **100**, SEO **66** |
| Playwright | **155 assertions** across 320 / 380 / 768 / 1440 |
| Internal links checked | 234, all resolve |

**SEO 66 is expected and is not a defect.** The only failing audit is
`is-crawlable`, which fires on the deliberate `noindex` that every pages.dev
response carries. It clears the moment `SITE_URL` moves to a real domain — no
other SEO audit fails.

Decisions taken during the build that are not in §1–§8:

| Decision | Reason |
|---|---|
| Shiki runs dual-theme with `defaultColor: false` | A single theme inlines `background-color` on every `<pre>`, which beat `--surface` and broke both the CSS-only scroll affordance and light mode. |
| The JS budget table distinguishes island from non-island pages | Astro inlines its own island bootstrap on any page with a `client:` directive. Collapsing the two into one row would have let an accidental island onto a prose page unnoticed. |
| `shiki` is a direct devDependency, pinned to Astro's own 4.4.3 | `MemoryStepper` needs the source split per line, which Astro's markdown pipeline does not expose. Same version, so it dedupes; nothing ships to the client. |
| Playground links use `edition=2024` | `cargo new` on cargo 1.97.1 writes `edition = "2024"`, so a learner copying a sample must land on the edition their own project uses. |
| The OG card is generated by Playwright, committed, and never built in CI | The card is Arabic, and Arabic needs GSUB/GPOS shaping. A browser already does that correctly with the site's own subset fonts. A Pages build that needs a browser is a build that fails while the author is away. |
...
| **Q-5** | ~~rustc pin.~~ **Resolved in practice: 1.97.1.** Every quoted message on the site was captured from `rustc 1.97.1 (8bab26f4f 2026-07-14)` / `cargo 1.97.1 (c980f4866 2026-06-30)` and is stamped with it. The standing question is only whether to RE-capture on each new stable; nothing is blocked until then. | — |
...
| **Q-7** | **Giscus `categoryId` — still open, and cannot be resolved here.** `repoId` is verified (`R_kgDOT2WV3g`, from the REST API) and Discussions are enabled on the repo. `categoryId` exists only in GitHub's GraphQL API, which always requires a token — the same limitation as E2 — so it was left empty rather than guessed. Until it is filled in, every comment section renders a link to Discussions and no third-party script loads. Fill it from giscus.app; it is a phone-sized task. Mapping is `specific`, not `pathname`: the term is the stable slug, so a URL change cannot orphan a thread. | live now |
...
| **Q-9** | **`/about` biography — still open.** The page is built and live; the biography block is a visibly marked placeholder. Name and GitHub handle are real (they come from the repo). Background, employer and projects are facts about a person, and inventing them would be the exact failure the site's accuracy rules exist to prevent. `CURRICULUM.md` §1 makes this the credibility mechanism, so it should not stay a placeholder past the domain switch. | domain switch |
